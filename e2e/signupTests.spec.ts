...
test('Account delete', async ({ page }) => {
  ...
  await page.getByRole('link', { name: 'Settings' }).waitFor({ state: 'visible' });
  await page.getByRole('link', { name: 'Settings' }).click();
  

  const deletePromice = page.waitForResponse(response => response.url().includes("/api/user/delete"));
  await page.getByRole('button', { name: 'Delete', exact: true }).click();
  const response = await deletePromice;
